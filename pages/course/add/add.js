const app = getApp()
const { addCourse, delCourse } = require('../../api/courseApi')
Page({
  ...
  save: function () {
    ...
    let courseWeekly_str = _this.data.weeklysResult
    let stringArr = courseWeekly_str.split(",");

    let elective_course_data = wx.getStorageSync('add_elective_course_data', "")
    let course_data_temp = wx.getStorageSync('course_data', "")
    if (!elective_course_data) {
      elective_course_data = []
    }

    for (const key in stringArr) {
      var course_weekly = stringArr[key]
      course_weekly = course_weekly + '-' + course_weekly

      let data = {
        // id: _this.data.id,
        courseName: _this.data.name,
        courseAddress: _this.data.address,
        teacherName: _this.data.teacher || '',
        courseWeekly: course_weekly + '周',
        currentWeek: _this.data.week + 1,
        courseSection: _this.data.section
      }
      elective_course_data.push(data)
      course_data_temp[0]['courseWeekly']['CourseData'].push(data)
    }
    console.log(elective_course_data);
    // wx.setStorageSync('course_data', course_data_temp);
    wx.setStorageSync('add_elective_course_data', elective_course_data);
